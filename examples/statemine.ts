...
import { alice, bob, Chain, ExtrinsicRune, Rune, RunicArgs, Sr25519, ValueRune } from "capi";
import { CollectionDetails, ItemDetails } from "http://localhost:4646/frame/zombienet/zombienets/statemine.toml/collator/@v0.9.370/types/pallet_uniques/types.ts";

const sendAndDebug = <X>(...[sender, prefix]: RunicArgs<X, [sender: Sr25519, prefix: String]>) => <U, C extends Chain = Chain>(extrinsic: ExtrinsicRune<U, C>) => {
...
const createCollection = Uniques.create({ collection: COLLECTION_ID, admin: alice.address })
...
const setCollectionMaxSupply = Uniques.setCollectionMaxSupply({ collection: COLLECTION_ID, maxSupply: 1 })

const alicePrepareCollection = Rune
  .chain(() => createCollection.pipe(sendAndDebug(alice, "Create Collection:")))
  .chain(() => setCollectionMetadata.pipe(sendAndDebug(alice, "Set Collection Metadata:")))
  .chain(() => mintAsset.pipe(sendAndDebug(alice, "Mint Asset:")))
  .chain(() => setAssetMetadata.pipe(sendAndDebug(alice, "Set Asset Metadata:")))
  .chain(() => setAssetPrice.pipe(sendAndDebug(alice, "Set Asset Price:")))
  .chain(() => setCollectionMaxSupply.pipe(sendAndDebug(alice, "Set Collection Max Supply:")));
...
) => collections.map(([first, ..._rest]) => first?.[0][0]).unhandle(undefined).into(ValueRune);
...
) => assets.map(([first, ..._rest]) => first?.[0][1]).unhandle(undefined).into(ValueRune);
  
const getItemPrice = <X>(...args: RunicArgs<X, [collectionId: number, assetId: number]>) => {
  return Uniques.ItemPriceOf.entry(Rune.tuple(args)).unhandle(undefined).map(([price]) => price)
}

const buyAsset = <X>(...[collectionId, assetId, bidPrice]: RunicArgs<X, [collectionId: number, assetId: number, bidPrice: bigint]>) => {
...
const bobBuyAsset = buyAsset(collectionId, assetId, price).pipe(sendAndDebug(bob, "Buying Asset:"))
...
const assetDetails = Uniques.Asset.entry(Rune.tuple([collectionId, assetId])).unhandle(undefined)
