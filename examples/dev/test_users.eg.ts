 ...
 * @stability nearing
 * @description When developing against a test chain, the `createTestUsers` utility
...
import { $, $sr25519, createTestUsers } from "capi"
...
const { alexa, billy, carol } = await createTestUsers()

// If count is specified, the result is an `Sr25519` tuple of the specified length.
const [david, ellie, felix] = await createTestUsers(3)
