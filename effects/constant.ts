...
import { constantMetadata, metadata, palletMetadata } from "./metadata.ts"
import * as scale from "./scale.ts"

export function constant<Client extends Z.$<rpc.Client>>(client: Client) {
  return <
  ...
  >(
    ...
    const constMetadata_ = constantMetadata(palletMetadata_, constName)
