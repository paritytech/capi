...
import { Primer, Run, Rune, RunicArgs, Unhandled } from "./Rune.ts"
...
class RunMatch<M, T, U> extends Run<T, U> {
  ...
  constructor(
    ...
    super(primer)
    this.value = primer.prime(child, this.signal)
    this.conditions = conditions.map(([cond, val]) =>
      [cond, primer.prime(val, this.signal)] as const
    )
  }
...
class RunHandle<T, T2 extends T, T3, U, U2> extends Run<Exclude<T, T2> | T3, U | U2> {
  ...
  constructor(
    ...
    super(primer)
    this.child = primer.prime(child, this.signal)
    this.alt = primer.prime(alt, this.signal)
  }
...
class RunGetUnhandled<T, U> extends Run<Unhandled<U> | null, never> {
  child
  constructor(primer: Primer, child: Rune<T, U>) {
    super(primer)
    this.child = primer.prime(child, this.signal)
  }
...
class RunFilter<T, U> extends Run<T, U> {
  child
  constructor(primer: Primer, child: Rune<T, U>, readonly fn: (value: T) => boolean) {
    super(primer)
    this.child = primer.prime(child, this.signal)
  }
...
class RunChain<T1, U1, T2, U2> extends Run<T2, U1 | U2> {
  ...
  constructor(
    ...
    super(primer)
    this.first = primer.prime(first, this.signal)
    this.second = primer.prime(second, this.signal)
  }
