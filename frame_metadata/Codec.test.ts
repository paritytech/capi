...
Deno.test("Derive AccountId32 Codec", async () => {
  ...
  const encoded = codec.encode(t.anyPairs.alice.publicKey);
  assertEquals(encoded, t.anyPairs.alice.publicKey);
  assertEquals(codec.decode(encoded), t.anyPairs.alice.publicKey);
});
...
Deno.test("Derive Auction Winning Storage Entry Codec", async () => {
  ...
  const decoded = [
    ...
    [t.anyPairs.alice.publicKey, 2013, 8672334557167609n],
