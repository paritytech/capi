import { alice, hex } from "../crypto/mod.ts"
...
Deno.test("Auction Winning Key", () => {
  const [pallet, storageEntry] = throwIfError(getPalletAndEntry(metadata, "Auctions", "Winning"))
  const $key = $storageKey({ deriveCodec, pallet, storageEntry })
  const key = [5]
  const encoded = $key.encode(key)
  assertEquals(
...
Deno.test("Multisig Multisigs partial storage Key", () => {
  const [pallet, storageEntry] = throwIfError(
