...
Deno.test("System Accounts Key", async () => {
  ...
  const key = encodeKey(deriveCodec, await getHashers(), pallet, storageEntry, accountId32);
...
Deno.test("Auction Winning Key", async () => {
  ...
  const key = encodeKey(deriveCodec, await getHashers(), pallet, storageEntry, 5);
  asserts.assertEquals(
    key,
