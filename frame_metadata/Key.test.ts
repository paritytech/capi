...
Deno.test("System Accounts Key", () => {
  ...
  const key = encodeKey(deriveCodec, hashersRuntime.hashers, pallet, storageEntry, accountId32);
...
Deno.test("Auction Winning Key", async () => {
  ...
  const key = encodeKey(deriveCodec, hashersRuntime.hashers, pallet, storageEntry, 5);
  asserts.assertEquals(
    key,
